...
class data_stack:
	...
	def avg(self):
		for i in range(len(self._val)):
			self._val[i] = self._val[i]/self._total
...
def callback(data):
	...
	#If a record is active then stack "calibration_time" _values into the current stack of the stack object
	if(stack.recording):
		if(stack._total < calibration_time):
			stack = stack + data
			print(data.data)
		else:
	...
	#Real time mode
		...
		if(stack._total < rt_mode_frequency):
			stack = stack + data
		else:
